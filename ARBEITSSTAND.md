...
- Zeitstrahl V1.5: Kartentexte vergrößert (Titel 14 px, Raum/Zuständigkeit und Datum 12 px, Zusatzzeile 11,5 px, Datumspillen 12,5 px), dafür Kartenhöhe 80 px und Zeilenabstand 90 px in `GEO`. Fläche wird bei 5 Zeilen je Seite 80 px höher, Breite unverändert; kein Text abgeschnitten, Kreuzungsbild unverändert. Nur `timeline.css`, `timeline-engine.js` (zwei Zahlen) und die Versionsangabe in `index.html` geändert, `data.json` unverändert. Grund: Druck ab Heute auf A3 quer soll bei Maßstab 1,0 Handout-Größe (10,5 pt Titel) erreichen. Nächster Schritt: Druckfenster ab Heute in `timeline-ui.js`.
